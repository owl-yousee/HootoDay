...
## Phase B-3f4 preparation: tombstone pull application design (2026-07-20)

- `hooto_day_pull_sync_records` returns the current `day_memo` row in change-sequence order. A tombstone is accepted only when `payload = null`, `deletedAt` is a valid timestamp, and revision/change sequence are valid positive integers; the pull result therefore provides the deleted date, deletion timestamp, revision, and change sequence without exposing deleted content.
- Incoming tombstones are classified as `remote_deleted_local_active`, `remote_deleted_local_modified`, `remote_deleted_local_missing`, or `remote_deleted_unknown`. An active local memo is eligible for deletion only when an active baseline exists, the local `updatedAt` still equals `baselineLocalUpdatedAt`, the remote tombstone is the direct successor of that baseline (`revision = baseline revision + 1` and a newer change sequence), the complete pull is valid, metadata/workspace/React/storage agree, and there is no pending operation, push block, or unresolved delete intent.
- A locally modified memo is never deleted automatically. It becomes a delete conflict and the local memo, remote tombstone, baseline, cursor, and pending state remain unchanged until a later user-decision phase. Missing or insufficient baseline evidence is `remote_deleted_unknown`, not a safe deletion.
- After a later explicit local application succeeds, the date baseline becomes a tombstone baseline containing the remote revision, change sequence, server-updated time and deleted time, with `baselineLocalUpdatedAt = null`; the cursor advances only after the DayMemo write and metadata write both pass read-back verification. The existing active baseline and local DayMemo must be restorable if either write fails.
- A pull row does not contain an operation ID or request fingerprint, so it cannot prove which device created a tombstone. A matching `localDeleteIntent` may support reconciliation when its base revision/change sequence and the remote successor match, but preview alone must not clear the intent. Any delete pending operation continues through the dedicated recovery path and is not handled as an ordinary incoming tombstone.
- A tombstone baseline is compatible with the normal safety state when metadata is valid and there is no pending operation, local delete intent, push block, conflict, or unexplained local memo for that date. Tombstone presence alone is not a danger state.
...
## Phase B-3f5e4d: verify recovery result and rebuild differences

- B-3f5e4d requires an unconsumed successful B-3f5e4d0a operation-result snapshot. One explicit action consumes that snapshot immediately before one complete read-only full pull; there is no automatic pull or retry.
- The target operation, pending, metadata v5, workspace/auth identity, recovery checkpoint, baselines/cursor, local DayMemo and canonical fingerprint, intent absence, and push-block absence are checked before and after the pull.
- The current active remote target must exactly match the saved operation result revision, change sequence, server updated time, state, and canonical payload. The local target must remain identical to the sent/result payload. A newer remote update or any mismatch stops fail-closed.
- The same complete pull is used to rebuild every date with the existing normal-difference classifier. The expected device state removes the resolved 2026-07-12 body mismatch while retaining the actual remaining differences, including the known body mismatch, local-only, and remote-only active dates. Results are never forced to expected counts.
- Only the resolved target receives an active baseline candidate. Existing baselines are preserved, the cursor candidate is the complete pull maximum sequence, and unresolved differences keep the candidate status `recovery_required`, confirmation time null, and normal-sync-ready false.
- Metadata v5 cannot persist the resolved target baseline while retaining its recovery pending. The read-only snapshot therefore records `clear_after_atomic_save` as the pending lifecycle candidate; this phase does not change or remove the pending or operation ID.
- The verification snapshot and candidate metadata exist in React state only. They are invalidated by changes to authentication/configuration, workspace, metadata, pending, checkpoint, baseline/cursor, local data, intent/push block, the operation-result snapshot token, remote/full-pull identity, or run identity.
- No DayMemo, metadata, pending, baseline, cursor, checkpoint, intent, push block, remote record, or operation history is changed. No mutation RPC or operation ID generation occurs. The next phase must explicitly and atomically save the candidate metadata and finalize the pending lifecycle.
## B-3f5e4d device-check correction: stable checkpoint handoff

- Device verification reached `checkpoint_unavailable` immediately after a successful B-3f5e4d0a operation-result read; the complete full pull correctly remained at zero calls.
- Root cause was an incompatible checkpoint fingerprint definition: B-3f5e4d0a included metadata version and workspace while B-3f5e4d recomputed a different shape without them. Identical persistent metadata therefore always compared unequal.
- Both hooks now share canonical fingerprint helpers for the recovery pending, sorted baselines, and persistent recovery checkpoint. Only stable persistent content participates; confirmation time, run ID, snapshot token, React identity, and object references do not participate in state equality.
...
## B-3f5e4d1 device-check correction: keep a fresh candidate ready

- Device verification reached `normal_body_mismatch_recovery_post_send_ready`, but the newly created save candidate was immediately reported as stale. No metadata, pending, baseline, cursor, DayMemo, local intent, push block, or remote state was changed.
- The exact cause was inconsistent pending fingerprint construction. Snapshot creation stored `JSON.stringify(pending)`, while freshness checks used the shared fixed-field canonical pending fingerprint. Equivalent pending content could therefore compare unequal solely because of property order.
- Snapshot creation now uses the same canonical pending fingerprint as every freshness check. Stable recursive fingerprints separately record the source metadata and candidate metadata: current persistent state is compared only with the source, while the candidate fingerprint and metadata v5 validator verify the object intended for saving.
