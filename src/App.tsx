...
import { useDayMemoBodyMismatchRemoteAdoption } from './hooks/useDayMemoBodyMismatchRemoteAdoption'
...
function App() {
  ...
  const dayMemoBodyMismatchRemoteAdoption = useDayMemoBodyMismatchRemoteAdoption({
    dayMemos,
    isConfigured: supabaseAuth.isConfigured,
    isSignedIn: supabaseAuth.isSignedIn,
    connection: supabaseWorkspace.connection,
    ...
    <div className="app-shell">
      <div className="app-body">
        ...
        <main className="main-content">
          {activeView === 'calendar' ? (
            ...
            <InventoryPage products={inventory.products} movements={inventory.inventoryMovements} eventSales={inventory.eventSalesRecords} boothSales={inventory.boothSalesRecords} boothWarehouseSales={inventory.boothWarehouseSalesRecords} anniversaryCampaigns={inventory.anniversaryCampaigns} anniversaryShipments={inventory.anniversaryShipments} events={events} initialEventId={inventoryEventId} onSaveProduct={inventory.saveProduct} onAddMovement={inventory.addMovement} onSaveEvent={inventory.saveEventSale} onSaveEventBatch={inventory.saveEventSalesBatch} onDeleteEvent={inventory.deleteEventSale} onSaveBooth={inventory.saveBoothSale} onDeleteBooth={inventory.deleteBoothSale} onSaveBoothWarehouse={inventory.saveBoothWarehouseSale} onDeleteBoothWarehouse={inventory.deleteBoothWarehouseSale} onSaveAnniversaryCampaign={inventory.saveAnniversaryCampaign} onSaveAnniversaryShipment={inventory.saveAnniversaryShipment} onSaveAnniversaryShipmentQrReference={inventory.saveAnniversaryShipmentQrReference} onDeleteAnniversaryShipment={inventory.deleteAnniversary} onDeleteAnniversaryCampaign={inventory.deleteAnniversaryCampaign} anniversaryQrConnection={{ isConfigured: supabaseAuth.isConfigured, isSignedIn: supabaseAuth.isSignedIn, workspaceId: supabaseWorkspace.connection?.workspaceId ?? null, workspaceConnected: Boolean(supabaseWorkspace.connection) }} onSaveCalendarEvent={saveEvent} syncCard={<InventorySyncCard sync={inventorySync} />} onEditingStateChange={setIsInventoryEditing} />
          ) : (
            <HealthExportPage
      ...
      {isThemeSettingsOpen && (
        <ThemeSettings
          ...
          dayMemoBodyMismatchRemoteAdoption={dayMemoBodyMismatchRemoteAdoption}
      ...
      {isDayMemoDialogOpen && (
        <DayMemoDialog
          ...
          onSendPreparedRemoteDelete={(date) => {
            if (dayMemoLocalOperationRemoteCheck.result?.date !== date
              || !dayMemoLocalOperationRemoteCheck.result.sendable) return
            void dayMemoLocalOperationSend.sendPreparedNormalDelete()
          }}
          onPrepareDeleteCleanup={(date) => {
            if (dayMemoLocalOperationSend.result?.date !== date
              || !dayMemoLocalOperationSend.result.remoteSucceeded
          ...
          deleteRemoteSendState={dayMemoLocalOperationSend.sending ? 'sending' : 'idle'}
