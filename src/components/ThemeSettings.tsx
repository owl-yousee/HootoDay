...
import { BODY_MISMATCH_LOCAL_ACTIONS, BODY_MISMATCH_REMOTE_ACTIONS,
  ...
  buildBodyMismatchCandidateUiModel, runBodyMismatchLocalAdoptionPipeline } from '../utils/dayMemoSyncActions'
...
import type { DayMemoBridgeNormalSaveSelection, useDayMemoNormalDifferenceRecoveryCheckpointCheck } from '../hooks/useDayMemoNormalDifferenceRecoveryCheckpointCheck'
...
interface NormalDifferenceRecoveryBridgeSnapshot {
  ...
  saveSelection: DayMemoBridgeNormalSaveSelection | null
}
...
type BodyMismatchComparisonRun = {
  ...
  remotePreparationCheck: {
    status: 'checking' | 'ready' | 'blocked' | 'failed'
    failureReason: string | null
    stopStage: string
    result: ReturnType<typeof useDayMemoBodyMismatchRemoteAdoption>['result']
    checkedAt: string | null
  } | null
  remoteFinalCheck: {
    status: 'checking' | 'ready' | 'blocked' | 'failed'
...
type BodyMismatchLocalFlow = {
  stage: 'idle' | 'applying' | 'applied' | 'finalizing' | 'complete' | 'blocked'
  safety: string | null
  date: string | null
  checkedAt: string | null
}
...
export function ThemeSettings({
  ...
  const [bodyMismatchLocalFlow, setBodyMismatchLocalFlow] = useState<BodyMismatchLocalFlow>({
    stage: 'idle', safety: null, date: null, checkedAt: null,
  })
  ...
  const runBodyMismatchComparison = async (date: string) => {
    ...
    if (dayMemoBodyMismatchRemoteAdoption.stage !== 'idle' && !dayMemoBodyMismatchRemoteAdoption.running) {
      dayMemoBodyMismatchRemoteAdoption.discard()
    }
    dayMemoNormalBodyMismatchCandidate.discard()
    setBodyMismatchLocalFlow({ stage: 'idle', safety: null, date: null, checkedAt: null })
  ...
  const confirmBodyMismatchCandidate = async (choice: 'local' | 'remote') => {
    const run = lastBodyMismatchComparisonRun
    if (!run || run.status !== 'ready' || !run.comparison
      || bodyMismatchComparisonRunIdRef.current !== run.runId
      || run.requestedDate !== recoveryNavigation.targetDate) return
    if (dayMemoBodyMismatchRemoteAdoption.running) return
    if (dayMemoBodyMismatchRemoteAdoption.stage !== 'idle') {
      dayMemoBodyMismatchRemoteAdoption.discard()
    }
    dayMemoNormalBodyMismatchLocalPreparation.discard()
    dayMemoBodyMismatchRecoveryPreflight.discard()
    dayMemoBodyMismatchRecoverySend.discard()
    dayMemoSavedOperationResultRead.discard()
    dayMemoBodyMismatchRecoveryPostSendVerification.discard()
    dayMemoBodyMismatchRecoveryCheckpointSave.discard()
    setBodyMismatchLocalFlow({ stage: 'idle', safety: null, date: run.requestedDate, checkedAt: null })
  ...
  const applyBodyMismatchLocalAdoption = async () => {
    ...
    if (!run || run.candidateCheck?.status !== 'ready' || run.candidateCheck.result?.candidate !== 'local'
      ...
      setBodyMismatchLocalFlow({ stage: 'blocked', safety: 'candidate_choice_mismatch',
        date: run?.requestedDate ?? null, checkedAt: new Date().toISOString() })
      return
    }
    if (!window.confirm(`${candidate.date} のlocal本文を同期先へ反映します。送信直前のread-only full pullと安全確認を行い、明示した1件だけを送信します。自動retryは行いません。実行しますか？`)) return
    setBodyMismatchLocalFlow({ stage: 'applying', safety: null, date: candidate.date, checkedAt: null })
    try {
      const sent = await runBodyMismatchLocalAdoptionPipeline({
        prepare: () => dayMemoNormalBodyMismatchLocalPreparation.prepare({ skipConfirmation: true }),
        getPreparationResult: dayMemoNormalBodyMismatchLocalPreparation.getLatestResult,
        checkPreflight: dayMemoBodyMismatchRecoveryPreflight.check,
        getPreflightResult: dayMemoBodyMismatchRecoveryPreflight.getLatestResult,
        send: () => dayMemoBodyMismatchRecoverySend.send({ skipConfirmation: true }),
        getSendResult: dayMemoBodyMismatchRecoverySend.getLatestResult,
        sendSucceeded: (result) => result.succeeded && result.rpcValidated && result.remoteUpdated,
        sendSafety: (result) => result.safety,
      })
      setBodyMismatchLocalFlow({ stage: 'applied', safety: sent.safety,
        date: candidate.date, checkedAt: sent.checkedAt })
    } catch (error) {
      setBodyMismatchLocalFlow({ stage: 'blocked',
        safety: error instanceof Error ? error.message : 'unknown',
        date: candidate.date, checkedAt: new Date().toISOString() })
    }
  }

  const finalizeBodyMismatchLocalAdoption = async () => {
    const run = lastBodyMismatchComparisonRun
    if (!run || bodyMismatchLocalFlow.stage !== 'applied'
      || bodyMismatchLocalFlow.date !== run.requestedDate) return
    if (!window.confirm(`${run.requestedDate} のlocal採用結果をread-backで確認し、baseline・cursor・metadataを確定します。自動retryは行いません。確定しますか？`)) return
    setBodyMismatchLocalFlow({ stage: 'finalizing', safety: null, date: run.requestedDate, checkedAt: null })
    try {
      await dayMemoSavedOperationResultRead.read({ skipConfirmation: true })
      const operationResult = dayMemoSavedOperationResultRead.getLatestResult()
      if (operationResult?.safety !== 'normal_body_mismatch_recovery_operation_result_read_ready'
        || !operationResult.snapshotCreated) throw new Error(operationResult?.safety ?? 'remote_result_invalid')
      await dayMemoBodyMismatchRecoveryPostSendVerification.check({ skipConfirmation: true })
      const verified = dayMemoBodyMismatchRecoveryPostSendVerification.getLatestResult()
      if (verified?.safety !== 'normal_body_mismatch_recovery_post_send_ready'
        || !verified.snapshotCreated || !verified.candidateNormalSyncReady
        || verified.unresolvedCount !== 0 || verified.candidateBaselineStatus !== 'confirmed') {
        throw new Error(verified?.safety ?? 'remote_read_back_failed')
      }
      dayMemoBodyMismatchRecoveryCheckpointSave.save({ skipConfirmation: true })
      const saved = dayMemoBodyMismatchRecoveryCheckpointSave.getLatestResult()
      if (!saved?.succeeded || !saved.normalSyncReady || saved.unresolvedCount !== 0
        || saved.baselineStatus !== 'confirmed' || saved.readBack !== 'succeeded') {
        throw new Error(saved?.safety ?? 'cleanup_read_back_failed')
      }
      setBodyMismatchLocalFlow({ stage: 'complete', safety: saved.safety,
        date: run.requestedDate, checkedAt: saved.checkedAt })
      dayMemoSavedRecoveryStateCheck.discard()
      setNormalSyncCheckUi({ status: 'success', stageId: 'normal_sync_complete', differenceCount: 0,
        normalSyncReady: true, message: 'local採用と同期情報の確定が完了しました。',
        nextAction: '同期作業を閉じる', checkedAt: saved.checkedAt })
    } catch (error) {
      setBodyMismatchLocalFlow({ stage: 'blocked',
        safety: error instanceof Error ? error.message : 'unknown',
        date: run.requestedDate, checkedAt: new Date().toISOString() })
    }
  }
      ...
      if (comparisonResult?.safety !== 'normal_body_mismatch_compare_ready'
        ...
        setLastBodyMismatchComparisonRun({ ...run, result: comparisonResult, comparison: refreshedComparison,
          ...
          remoteContentCheck: { status: comparisonResult ? 'blocked' : 'failed',
            ...
            checkedAt: new Date().toISOString() }, remotePreparationCheck: null, remoteFinalCheck: null })
        return
      }
  ...
  const prepareBodyMismatchRemoteAdoption = async () => {
    const run = lastBodyMismatchComparisonRun
    ...
    if (!run || run.status !== 'ready' || !content || content.status !== 'ready'
      ...
      || content.snapshotRevision == null) {
      if (run) setLastBodyMismatchComparisonRun({ ...run, remotePreparationCheck: {
        status: 'blocked', failureReason: !content ? 'remote_content_check_missing' : 'remote_content_check_not_ready',
        stopStage: 'remote_adoption_preparation_start', result: null, checkedAt: new Date().toISOString(),
      }, remoteFinalCheck: null })
      return
    }
    setLastBodyMismatchComparisonRun({ ...run, remotePreparationCheck: {
      status: 'checking', failureReason: null, stopStage: 'remote_adoption_preparation_checking',
      result: null, checkedAt: null,
    }, remoteFinalCheck: null })
    try {
      const result = await dayMemoBodyMismatchRemoteAdoption.prepareRemote({
        date: run.requestedDate, comparisonRunId: run.runId,
        candidateRevision: content.candidateRevision, contentRevision: content.snapshotRevision,
      })
      if (bodyMismatchComparisonRunIdRef.current !== run.runId) return
      const snapshot = dayMemoBodyMismatchRemoteAdoption.getPreparationSnapshot()
      const ready = result?.stage === 'preparation_ready'
        && result.preparationCreated && snapshot?.preparationRevision === result.preparationRevision
        && snapshot.comparisonRunId === run.runId && snapshot.contentRevision === content.snapshotRevision
      setLastBodyMismatchComparisonRun({ ...run, remotePreparationCheck: {
        status: ready ? 'ready' : result ? 'blocked' : 'failed',
        failureReason: ready ? null : result?.safety ?? 'unknown',
        stopStage: ready ? 'remote_adoption_preparation_ready' : result?.safety ?? 'remote_adoption_preparation_failed',
        result, checkedAt: new Date().toISOString(),
      }, remoteFinalCheck: null })
    } catch {
      if (bodyMismatchComparisonRunIdRef.current !== run.runId) return
      setLastBodyMismatchComparisonRun({ ...run, remotePreparationCheck: {
        status: 'failed', failureReason: 'unknown', stopStage: 'remote_adoption_preparation_unexpected_failure',
        result: null, checkedAt: new Date().toISOString(),
      }, remoteFinalCheck: null })
    }
  }

  const verifyBodyMismatchRemoteAdoptionExecution = async () => {
    const run = lastBodyMismatchComparisonRun
    const content = run?.remoteContentCheck
    const preparationCheck = run?.remotePreparationCheck
    ...
    if (!run || run.status !== 'ready' || !content || content.status !== 'ready'
      ...
      || run.requestedDate !== recoveryNavigation.targetDate
      || content.comparisonRunId !== run.runId || content.candidateRevision == null
      || content.snapshotRevision == null || preparationCheck.result.preparationRevision == null
      || !preparationSnapshot
      || preparationSnapshot.preparationRevision !== preparationCheck.result.preparationRevision) {
      if (run) setLastBodyMismatchComparisonRun({ ...run, remoteFinalCheck: {
        status: 'blocked', failureReason: !preparationSnapshot ? 'remote_preparation_missing' : 'remote_preparation_not_ready',
        stopStage: 'remote_adoption_execution_verification_start', result: null, checkedAt: new Date().toISOString(),
      } })
      return
    }
    setLastBodyMismatchComparisonRun({ ...run, remoteFinalCheck: {
      status: 'checking', failureReason: null, stopStage: 'remote_adoption_execution_verification_checking',
      result: null, checkedAt: null,
    } })
    try {
      const result = await dayMemoBodyMismatchRemoteAdoption.verifyPreparedRemote({
        date: run.requestedDate, comparisonRunId: run.runId,
        candidateRevision: content.candidateRevision, contentRevision: content.snapshotRevision,
        preparationRevision: preparationCheck.result.preparationRevision,
      })
      if (bodyMismatchComparisonRunIdRef.current !== run.runId) return
      const executionSnapshot = dayMemoBodyMismatchRemoteAdoption.getExecutionSnapshot()
      const ready = result?.stage === 'execution_ready'
        && result.finalVerificationRevision != null
        && executionSnapshot?.finalVerificationRevision === result.finalVerificationRevision
        && executionSnapshot.preparation.preparationRevision === preparationCheck.result.preparationRevision
      setLastBodyMismatchComparisonRun({ ...run, remoteFinalCheck: {
        status: ready ? 'ready' : result ? 'blocked' : 'failed',
        failureReason: ready ? null : result?.safety ?? 'unknown',
        stopStage: ready ? 'remote_adoption_execution_verification_ready'
          : result?.safety ?? 'remote_adoption_execution_verification_failed',
        result, checkedAt: new Date().toISOString(),
      } })
    } catch {
      if (bodyMismatchComparisonRunIdRef.current !== run.runId) return
      setLastBodyMismatchComparisonRun({ ...run, remoteFinalCheck: {
        status: 'failed', failureReason: 'unknown',
        stopStage: 'remote_adoption_execution_verification_unexpected_failure',
        result: null, checkedAt: new Date().toISOString(),
      } })
    }
  }
  ...
  const bodyMismatchCandidateUiModel = buildBodyMismatchCandidateUiModel(
    lastBodyMismatchComparisonRun?.candidateCheck?.status,
    lastBodyMismatchComparisonRun?.candidateCheck?.result,
    dayMemoNormalBodyMismatchCandidate.getCandidateSnapshot(),
  )
  ...
  const prepareNormalDifferenceRecoveryBridgeSave = () => {
    if (!normalDifferenceRecoveryBridge) return
                                        ...
                                        {normalDifferenceRecoveryBridge.savePreparationRequested
                                          && dayMemoNormalDifferenceRecoveryCheckpointCheck.savePreparationResult ? (() => {
                                            ...
                                            return <div className={`cloud-day-memo-preview-result ${preparationReady ? '' : 'is-blocked'}`}
                                              ...
                                              {preparationReady && normalDifferenceRecoveryBridge.saveSelection ? (
                                                <div className="cloud-day-memo-preview-result" role="status">
                                                  <h6>checkpoint保存実行準備</h6>
                                                  <ul className="cloud-day-memo-preview-summary">
                                                    <li>checkpoint方式：{normalDifferenceRecoveryBridge.saveSelection.checkpointMethod}</li>
                                                    <li>cursor変更予定：{normalDifferenceRecoveryBridge.saveSelection.currentCursor} → {normalDifferenceRecoveryBridge.saveSelection.candidateCursor}</li>
                                                    <li>baseline追加予定：{normalDifferenceRecoveryBridge.saveSelection.baselineAdditionCount}件</li>
                                                    <li>未解決差異：{normalDifferenceRecoveryBridge.saveSelection.unresolvedCount}件</li>
                                                    <li>保存対象metadata：React stateへ保持済み</li>
                                                    <li>永続保存：未実行</li>
                                                  </ul>
                                                  {Object.entries(normalDifferenceRecoveryBridge.saveSelection.unresolvedClassifications).length > 0 ? (
                                                    <ul className="cloud-day-memo-preview-summary">
                                                      {Object.entries(normalDifferenceRecoveryBridge.saveSelection.unresolvedClassifications)
                                                        .map(([date, classification]) => (
                                                          <li key={`save-selection:${date}`}>{date}：{classification}</li>
                                                        ))}
                                                    </ul>
                                                  ) : null}
                      ...
                      {lastBodyMismatchComparisonRun ? (
                        <div className={`cloud-day-memo-preview-result is-${lastBodyMismatchComparisonRun.status}`}
                          ...
                          {lastBodyMismatchComparisonRun.status === 'checking' ? (
                            ...
                            {!bodyMismatchLocalFlowActive ? <div className="iphone-sync-guide-actions">
                              <button type="button" className="health-secondary-button cloud-sync-button"
                            ...
                            {lastBodyMismatchComparisonRun.candidateCheck ? (
                              <div className={`cloud-day-memo-preview-result is-${lastBodyMismatchComparisonRun.candidateCheck.status}`}
                                ...
                                <h6>{bodyMismatchLocalFlowActive ? 'local採用処理'
                                  : bodyMismatchCandidateUiModel?.title
                                  ?? `採用候補確認：${lastBodyMismatchComparisonRun.candidateCheck.status}`}</h6>
                                {!bodyMismatchLocalFlowActive ? <ul className="cloud-day-memo-preview-summary">
                                  <li>対象：{lastBodyMismatchComparisonRun.requestedDate}</li>
                                  ...
                                  <li>local変更：未実行</li><li>remote変更：未実行</li>
                                  <li>metadata変更：未実行</li><li>baseline変更：未実行</li><li>cursor変更：未実行</li>
                                  <li>pending作成：なし</li><li>intent作成：なし</li><li>operation ID生成：なし</li>
                                  <li>Supabase書き込み：なし</li><li>永続変更：なし</li><li>自動retry：なし</li>
                                  {lastBodyMismatchComparisonRun.candidateCheck.result?.failureReason ? (
                                    <li>failureReason：{lastBodyMismatchComparisonRun.candidateCheck.result.failureReason}</li>
                                  ) : null}
                                </ul> : null}
                                {lastBodyMismatchComparisonRun.candidateCheck.status === 'ready' ? (
                                  <>
                                    {!bodyMismatchLocalFlowActive && bodyMismatchCandidateUiModel?.choice === 'local' ? <>
                                    ...
                                    </> : bodyMismatchCandidateUiModel?.choice === 'remote' ? <>
                                      <label>現在local本文<textarea readOnly rows={4}
                                        value={lastBodyMismatchComparisonRun.comparison.localContent} /></label>
                                      <label>採用予定remote本文<textarea readOnly rows={4}
                                        value={lastBodyMismatchComparisonRun.comparison.remoteContent} /></label>
                                    </> : null}
                                    {!bodyMismatchLocalFlowActive
                                      ? <p>次操作：{bodyMismatchCandidateUiModel?.nextAction ?? '候補を再確認'}</p> : null}
                                    {!bodyMismatchLocalFlowActive && bodyMismatchCandidateUiModel?.showRemoteAction ? (
                                      <button type="button" className="health-primary-button cloud-sync-button"
                                    ...
                                    ) : !bodyMismatchLocalFlowActive && bodyMismatchCandidateUiModel?.showLocalAction ? (
                                      <button type="button" className="health-primary-button cloud-sync-button"
                                        disabled={bodyMismatchLocalFlow.stage === 'applying'
                                          || bodyMismatchLocalFlow.stage === 'finalizing'}
                                        onClick={() => { void applyBodyMismatchLocalAdoption() }}>
                                      ...
                                      && bodyMismatchLocalFlow.stage !== 'idle' ? (
                                        <div className={`cloud-day-memo-preview-result is-${
                                          bodyMismatchLocalFlow.stage === 'complete' || bodyMismatchLocalFlow.stage === 'applied'
                                            ? 'ready' : bodyMismatchLocalFlow.stage === 'blocked' ? 'blocked' : 'checking'}`}
                                          role={bodyMismatchLocalFlow.stage === 'blocked' ? 'alert' : 'status'}>
                                          <h6>local採用：{bodyMismatchLocalFlow.stage === 'applied' ? '送信完了'
                                            : bodyMismatchLocalFlow.stage === 'complete' ? '完了'
                                              : bodyMismatchLocalFlow.stage}</h6>
                                          <ul className="cloud-day-memo-preview-summary">
                                            <li>対象：{bodyMismatchLocalFlow.date ?? '未確認'}</li>
                                            <li>採用元：local</li>
                                            <li>safety：{bodyMismatchLocalFlow.safety ?? '確認中'}</li>
                                            <li>自動retry：なし</li>
                                            <li>operation ID：既存lifecycle内で1回だけ生成</li>
                                            {bodyMismatchLocalFlow.stage === 'applied' ? <>
                                              <li>remote RPC：成功</li>
                                              <li>RPC結果検証：成功</li>
                                              <li>remote反映：完了</li>
                                              <li>pending：保持中</li>
                                              <li>metadata cleanup：未実行</li>
                                              <li>差異：未確認</li>
                                              <li>Supabase書き込み：あり</li>
                                            </> : null}
                                          </ul>
                                          {bodyMismatchLocalFlow.stage === 'applied' ? <>
                                            <p>RPC結果検証まで完了しました。metadata cleanupは未実行です。</p>
                                            <button type="button" className="health-primary-button cloud-sync-button"
                                              onClick={() => { void finalizeBodyMismatchLocalAdoption() }}>
                                              {BODY_MISMATCH_LOCAL_ACTIONS.finalize}
                                            </button>
                                          </> : null}
                                          {bodyMismatchLocalFlow.stage === 'finalizing'
                                            ? <p>{BODY_MISMATCH_LOCAL_ACTIONS.finalizing}</p> : null}
                                          {bodyMismatchLocalFlow.stage === 'complete'
                                            ? <p>local採用結果を確定しました。差異0件・通常同期readyです。</p> : null}
                                          {bodyMismatchLocalFlow.stage === 'blocked'
                                            ? <p>現在段階を再確認するか、保存済み状態からやり直してください。</p> : null}
                                        </div>
                                      ) : null}
                                  </>
                                  ...
                                  <div className={`cloud-day-memo-preview-result is-${lastBodyMismatchComparisonRun.remoteContentCheck.status}`}
                                    ...
                                    {lastBodyMismatchComparisonRun.remoteContentCheck.status === 'checking' ? (
                                      ...
                                      {lastBodyMismatchComparisonRun.remoteContentCheck.status === 'ready' ? <>
                                        ...
                                        <button type="button" className="health-primary-button cloud-sync-button"
                                          disabled={lastBodyMismatchComparisonRun.remotePreparationCheck?.status === 'checking'}
                                          onClick={() => { void prepareBodyMismatchRemoteAdoption() }}>
                                          {lastBodyMismatchComparisonRun.remotePreparationCheck?.status === 'checking'
                                            ? 'remote採用準備を確認中…' : 'remote採用準備を開始'}
                                        </button>
                                ...
                                {lastBodyMismatchComparisonRun.remotePreparationCheck ? (
                                  <div className={`cloud-day-memo-preview-result is-${lastBodyMismatchComparisonRun.remotePreparationCheck.status}`}
                                    role={lastBodyMismatchComparisonRun.remotePreparationCheck.status === 'ready'
                                      || lastBodyMismatchComparisonRun.remotePreparationCheck.status === 'checking' ? 'status' : 'alert'}>
                                    <h6>remote採用準備：{lastBodyMismatchComparisonRun.remotePreparationCheck.status}</h6>
                                    {lastBodyMismatchComparisonRun.remotePreparationCheck.status === 'checking' ? (
                                      <p>最新状態を読み取り専用で確認し、採用準備snapshotを作成しています…</p>
                                    ) : <>
                                      <ul className="cloud-day-memo-preview-summary">
                                        <li>対象：{lastBodyMismatchComparisonRun.requestedDate}</li>
                                        <li>採用元：remote</li>
                                        <li>比較runId：{lastBodyMismatchComparisonRun.remotePreparationCheck.result?.comparisonRunId ?? lastBodyMismatchComparisonRun.runId}</li>
                                        <li>candidate revision：{lastBodyMismatchComparisonRun.remotePreparationCheck.result?.candidateRevision ?? '未確認'}</li>
                                        <li>content snapshot revision：{lastBodyMismatchComparisonRun.remotePreparationCheck.result?.contentRevision ?? '未確認'}</li>
                                        <li>preparation revision：{lastBodyMismatchComparisonRun.remotePreparationCheck.result?.preparationRevision ?? '未生成'}</li>
                                        <li>metadata cursor：{lastBodyMismatchComparisonRun.metadataCursor ?? '未確認'}</li>
                                        <li>metadata：{lastBodyMismatchComparisonRun.remotePreparationCheck.result?.metadataVerified ? '一致' : '未確認／不一致'}</li>
                                        <li>workspace：{lastBodyMismatchComparisonRun.remotePreparationCheck.result?.workspaceVerified ? '一致' : '未確認／不一致'}</li>
                                        <li>local snapshot：{lastBodyMismatchComparisonRun.remotePreparationCheck.result?.localVerified ? '一致' : '未確認／不一致'}</li>
                                        <li>remote identity：{lastBodyMismatchComparisonRun.remotePreparationCheck.result?.remoteVerified ? '一致' : '未確認／不一致'}</li>
                                        <li>baseline identity：{lastBodyMismatchComparisonRun.remotePreparationCheck.result?.baselineVerified ? '一致' : '未確認／不一致'}</li>
                                        <li>cursor／full pull：{lastBodyMismatchComparisonRun.remotePreparationCheck.result?.cursorVerified ? '一致' : '未確認／不一致'}</li>
                                        <li>未解決差異：{lastBodyMismatchComparisonRun.remotePreparationCheck.result?.differencesVerified ? '不変' : '未確認／変化あり'}</li>
                                        <li>local変更：未実行</li><li>remote変更：未実行</li>
                                        <li>metadata変更：未実行</li><li>baseline／cursor変更：未実行</li>
                                        <li>pending作成：なし</li><li>intent作成：なし</li><li>operation ID生成：なし</li>
                                        <li>Supabase書き込み：なし</li><li>自動retry：なし</li>
                                        {lastBodyMismatchComparisonRun.remotePreparationCheck.failureReason ? (
                                          <li>failureReason：{lastBodyMismatchComparisonRun.remotePreparationCheck.failureReason}</li>
                                        ) : null}
                                        <li>停止段階：{lastBodyMismatchComparisonRun.remotePreparationCheck.stopStage}</li>
                                      </ul>
                                      {lastBodyMismatchComparisonRun.remotePreparationCheck.status === 'ready' ? <>
                                        <label>現在local本文<textarea readOnly rows={6}
                                          value={lastBodyMismatchComparisonRun.remoteContentCheck?.comparison?.localContent ?? ''} /></label>
                                        <label>採用予定remote本文<textarea readOnly rows={6}
                                          value={lastBodyMismatchComparisonRun.remoteContentCheck?.comparison?.remoteContent ?? ''} /></label>
                                        <p>採用後の予定：local本文をremote本文へ置き換え、verified read-back後に別操作でmetadataを整理します。</p>
                                        <p>次操作：remote採用実行前の最終確認</p>
                                        <button type="button" className="health-primary-button cloud-sync-button"
                                          disabled={lastBodyMismatchComparisonRun.remoteFinalCheck?.status === 'checking'}
                                          onClick={() => { void verifyBodyMismatchRemoteAdoptionExecution() }}>
                                          {lastBodyMismatchComparisonRun.remoteFinalCheck?.status === 'checking'
                                            ? 'remote採用実行前の最終確認中…' : 'remote採用実行前の最終確認'}
                                        </button>
                                      </> : <div className="iphone-sync-guide-actions">
                                        <button type="button" className="health-primary-button cloud-sync-button"
                                          onClick={() => { void prepareBodyMismatchRemoteAdoption() }}>remote採用準備を再確認</button>
                                        <button type="button" className="health-secondary-button cloud-sync-button"
                                          onClick={() => { void verifyBodyMismatchRemoteAdoptionContent() }}>remote採用内容から再確認</button>
                                        <button type="button" className="health-secondary-button cloud-sync-button"
                                          onClick={() => { void confirmBodyMismatchCandidate('remote') }}>remote採用候補から再確認</button>
                                      </div>}
                                    </>}
                                  </div>
                                ) : null}
                                {lastBodyMismatchComparisonRun.remoteFinalCheck ? (
                                  <div className={`cloud-day-memo-preview-result is-${lastBodyMismatchComparisonRun.remoteFinalCheck.status}`}
                                    role={lastBodyMismatchComparisonRun.remoteFinalCheck.status === 'ready'
                                      || lastBodyMismatchComparisonRun.remoteFinalCheck.status === 'checking' ? 'status' : 'alert'}>
                                    <h6>remote採用実行前確認：{lastBodyMismatchComparisonRun.remoteFinalCheck.status}</h6>
                                    {lastBodyMismatchComparisonRun.remoteFinalCheck.status === 'checking' ? (
                                      <p>local反映直前の状態を読み取り専用で確認しています…</p>
                                    ) : <>
                                      <ul className="cloud-day-memo-preview-summary">
                                        <li>対象：{lastBodyMismatchComparisonRun.requestedDate}</li>
                                        <li>採用元：remote</li>
                                        <li>比較runId：{lastBodyMismatchComparisonRun.remoteFinalCheck.result?.comparisonRunId ?? lastBodyMismatchComparisonRun.runId}</li>
                                        <li>candidate revision：{lastBodyMismatchComparisonRun.remoteFinalCheck.result?.candidateRevision ?? '未確認'}</li>
                                        <li>content revision：{lastBodyMismatchComparisonRun.remoteFinalCheck.result?.contentRevision ?? '未確認'}</li>
                                        <li>preparation revision：{lastBodyMismatchComparisonRun.remoteFinalCheck.result?.preparationRevision ?? '未確認'}</li>
                                        <li>final verification revision：{lastBodyMismatchComparisonRun.remoteFinalCheck.result?.finalVerificationRevision ?? '未生成'}</li>
                                        <li>metadata cursor：{lastBodyMismatchComparisonRun.metadataCursor ?? '未確認'}</li>
                                        <li>metadata：{lastBodyMismatchComparisonRun.remoteFinalCheck.result?.metadataVerified ? '一致' : '未確認／不一致'}</li>
                                        <li>workspace：{lastBodyMismatchComparisonRun.remoteFinalCheck.result?.workspaceVerified ? '一致' : '未確認／不一致'}</li>
                                        <li>local storage／React：{lastBodyMismatchComparisonRun.remoteFinalCheck.result?.localVerified ? '一致' : '未確認／不一致'}</li>
                                        <li>remote identity：{lastBodyMismatchComparisonRun.remoteFinalCheck.result?.remoteVerified ? '一致' : '未確認／不一致'}</li>
                                        <li>baseline identity：{lastBodyMismatchComparisonRun.remoteFinalCheck.result?.baselineVerified ? '一致' : '未確認／不一致'}</li>
                                        <li>cursor／full pull：{lastBodyMismatchComparisonRun.remoteFinalCheck.result?.cursorVerified ? '一致' : '未確認／不一致'}</li>
                                        <li>body mismatch／対象外差異：{lastBodyMismatchComparisonRun.remoteFinalCheck.result?.differencesVerified ? '確認済み・不変' : '未確認／変化あり'}</li>
                                        <li>対象外local：変更予定なし</li>
                                        <li>local変更：未実行</li><li>remote変更：未実行</li>
                                        <li>metadata変更：未実行</li><li>baseline／cursor変更：未実行</li>
                                        <li>pending作成：なし</li><li>intent作成：なし</li><li>operation ID生成：なし</li>
                                        <li>Supabase書き込み：なし</li><li>自動retry：なし</li>
                                        {lastBodyMismatchComparisonRun.remoteFinalCheck.failureReason ? (
                                          <li>failureReason：{lastBodyMismatchComparisonRun.remoteFinalCheck.failureReason}</li>
                                        ) : null}
                                        <li>停止段階：{lastBodyMismatchComparisonRun.remoteFinalCheck.stopStage}</li>
                                      </ul>
                                      <label>現在local本文<textarea readOnly rows={6}
                                        value={lastBodyMismatchComparisonRun.remoteContentCheck?.comparison?.localContent ?? ''} /></label>
                                      <label>反映予定remote本文<textarea readOnly rows={6}
                                        value={lastBodyMismatchComparisonRun.remoteContentCheck?.comparison?.remoteContent ?? ''} /></label>
