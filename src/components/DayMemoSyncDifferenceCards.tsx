...
type DifferenceUiAction = {
  id: 'inspect' | 'adopt_remote' | 'upload_local' | 'discard_local' | 'hold' | 'safety_check'
  label: string
}

const REMOTE_ONLY_ACTIONS: DifferenceUiAction[] = [
  { id: 'adopt_remote', label: 'このiPhoneへ反映' },
  { id: 'hold', label: '今回は保留' },
]
const LOCAL_ONLY_ACTIONS: DifferenceUiAction[] = [
  { id: 'upload_local', label: '同期先へ送る' },
  { id: 'discard_local', label: 'このiPhoneから削除' },
  { id: 'hold', label: '今回は保留' },
]
const BODY_MISMATCH_ACTIONS: DifferenceUiAction[] = [
  { id: 'hold', label: '今回は保留' },
]
const SAFETY_ACTIONS: DifferenceUiAction[] = [
  { id: 'hold', label: '今回は保留' },
]

function actionsFor(classification: string): DifferenceUiAction[] {
...
function entryActionFor(classification: string): DifferenceUiAction {
...
export function DayMemoSyncDifferenceCards({ items, stopReason }: Props) {
  ...
  const [selectedAction, setSelectedAction] = useState<DifferenceUiAction | null>(null)
