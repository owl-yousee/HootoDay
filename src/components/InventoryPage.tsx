...
import type { AnniversaryCampaign, AnniversaryShipment, BoothSalesRecord, BoothWarehouseSaleRecord, EventSalesRecord, InventoryMovement, Product } from '../types/inventory';
...
import { BoothWarehouseSalesPanel } from './BoothWarehouseSalesPanel';
import { AnniversaryManagementPanel } from './AnniversaryManagementPanel';
...
interface Props {
    ...
    onDeleteBooth: (id: string) => string | null;
    onSaveBoothWarehouse: (value: BoothWarehouseSaleRecord, expectedExistingId?: string | null) => string | null;
    onDeleteBoothWarehouse: (id: string) => string | null;
    onSaveAnniversaryCampaign: (campaign: AnniversaryCampaign, expectedCampaignId?: string | null) => string | null;
    onSaveAnniversaryShipment: (shipment: AnniversaryShipment, expectedShipmentId?: string | null) => string | null;
    onSaveAnniversaryShipmentQrReference: (shipment: AnniversaryShipment, expectedShipmentId: string) => string | null;
    onDeleteAnniversaryShipment: (shipmentId: string) => string | null;
    onDeleteAnniversaryCampaign: (campaignId: string) => string | null;
    anniversaryQrConnection: {
...
export function InventoryPage(props: Props) {
    ...
    const [boothTab, setBoothTab] = useState<BoothTab>('warehouse');
    ...
    const [boothStatus, setBoothStatus] = useState<BoothSalesRecord['status']>('pending');
    const [isBoothSubmitting, setIsBoothSubmitting] = useState(false);
    ...
    const boothSubmitInProgressRef = useRef(false);
    ...
    const open = (next: Mode, product: Product | null = null, event: EventSalesRecord | null = null, booth: BoothSalesRecord | null = null) => {
        ...
        setBoothStatus(booth?.status ?? 'pending');
        ...
        boothSubmitInProgressRef.current = false;
        setIsBoothSubmitting(false);
    ...
    const close = () => { dialogRef.current?.close(); setIsDialogOpen(false); onEditingStateChange?.(false); setEditingEvent(null); setEditingBooth(null); editingBoothIdRef.current = null; movementSubmitInProgressRef.current = false; boothSubmitInProgressRef.current = false; setIsMovementSubmitting(false); setIsBoothSubmitting(false); setError(''); setEventFieldErrors({}); setMovementFieldErrors({}); };
    ...
    const submit = (submitEvent: React.FormEvent<HTMLFormElement>) => {
        ...
        const quantityRaw = String(data.get('quantity') ?? '').trim();
        const priceRaw = String(data.get('unitPrice') ?? '').trim();
        const shippingFeeRaw = String(data.get('shippingFee') ?? '').trim();
        const quantity = Number(quantityRaw), price = Number(priceRaw);
        const shippingFee = shippingFeeRaw === '' ? null : Number(shippingFeeRaw);
        const orderReference = String(data.get('orderReference') ?? '').trim();
        const memo = String(data.get('memo') ?? '').trim();
        const date = String(data.get('date') ?? '');
        const shippedAt = boothStatus === 'shipped' ? String(data.get('shippedAt') ?? '') || null : null;
        if (!fromDateKey(date)) {
        ...
        if (!/^\d+$/.test(priceRaw) || !Number.isSafeInteger(price) || price < 0) {
            setError('販売価格を0以上の整数で入力してください。');
            return;
        }
        if (shippingFee !== null && (!/^\d+$/.test(shippingFeeRaw) || !Number.isSafeInteger(shippingFee) || shippingFee < 0)) {
            setError('送料は0以上の整数で入力してください。');
            return;
        }
        if (shippedAt && !fromDateKey(shippedAt)) {
            setError('発送日が正しくありません。');
            return;
        }
        if (orderReference.length > 100 || memo.length > 500) {
            setError('注文番号は100文字以内、メモは500文字以内で入力してください。');
            return;
        }
        ...
        if (boothSubmitInProgressRef.current)
            return;
        const boothRecordId = existingBooth?.id ?? createUuidV4();
        if (!boothRecordId) {
            setError('保存に必要なIDを作成できませんでした。入力内容は変更されていません。');
            return;
        }
        boothSubmitInProgressRef.current = true;
        setIsBoothSubmitting(true);
        const result = props.onSaveBooth({ id: boothRecordId, date, productId: existingBooth?.productId ?? productId, productNameSnapshot: existingBooth?.productNameSnapshot ?? (product.boothDisplayName || product.name), unitPriceSnapshot: price, quantity, orderReference, status: boothStatus, shippingFee, shippedAt, memo, createdAt: existingBooth?.createdAt ?? now, updatedAt: now }, editingBoothId);
        if (result) {
            boothSubmitInProgressRef.current = false;
            setIsBoothSubmitting(false);
            setError(result);
    ...
    const activeBoothSales = props.boothSales.filter((item) => item.status !== 'cancelled');
    const totalBoothQuantity = activeBoothSales.reduce((sum, item) => sum + item.quantity, 0);
    const totalBooth = activeBoothSales.reduce((sum, item) => sum + item.quantity * item.unitPriceSnapshot, 0);
    const totalBoothShippingFee = activeBoothSales.reduce((sum, item) => sum + (item.shippingFee ?? 0), 0);
    ...
    {tab === 'booth' && <><nav className="inventory-booth-tabs" aria-label="BOOTH販売方式"><button type="button" aria-pressed={boothTab === 'warehouse'} onClick={() => setBoothTab('warehouse')}>倉庫</button><button type="button" aria-pressed={boothTab === 'home'} onClick={() => setBoothTab('home')}>家発送</button></nav>{boothTab === 'warehouse'
      ? <BoothWarehouseSalesPanel products={props.products} movements={props.movements} records={props.boothWarehouseSales} onSave={props.onSaveBoothWarehouse} onDelete={props.onDeleteBoothWarehouse} onEditingStateChange={onEditingStateChange}/>
      : <section className="inventory-section"><div className="inventory-section-heading"><div><h2>BOOTH家発送</h2><p>自宅から発送するBOOTH注文を管理します。購入者名・住所・メールは保存しません。</p></div><button className="health-primary-button" disabled={!props.products.some(item => item.isActive && item.boothEnabled)} onClick={() => open('booth')}>家発送を登録</button></div><div className="inventory-home-shipping-totals"><span>販売数 <strong>{totalBoothQuantity.toLocaleString('ja-JP')}個</strong></span><span>商品売上 <strong>{money(totalBooth)}</strong></span><span>送料 <strong>{money(totalBoothShippingFee)}</strong></span></div>{[...props.boothSales].sort((a, b) => b.date.localeCompare(a.date) || b.updatedAt.localeCompare(a.updatedAt)).map(item => <article className={`inventory-row inventory-booth-row${item.status === 'cancelled' ? ' is-cancelled' : ''}`} key={item.id}><strong>{item.productNameSnapshot}</strong><div className="inventory-booth-detail"><span>{item.date}・{item.status === 'pending' ? '未発送' : item.status === 'shipped' ? '発送済み' : 'キャンセル'}・{item.quantity}個</span><small>商品売上 {money(item.quantity * item.unitPriceSnapshot)} / 送料 {item.shippingFee === null ? '未設定' : money(item.shippingFee)}</small>{item.orderReference && <small>注文番号：{item.orderReference}</small>}{item.shippedAt && <small>発送日：{item.shippedAt}</small>}{item.memo && <small>{item.memo}</small>}</div><span>{item.status === 'cancelled' ? '集計対象外' : money(item.quantity * item.unitPriceSnapshot)}</span><div className="inventory-row-actions"><button type="button" onClick={() => open('booth', props.products.find(product => product.id === item.productId) ?? null, null, item)}>編集</button><button type="button" className="inventory-danger-button" onClick={() => { if (!window.confirm(`この家発送記録を削除しますか？${item.status === 'cancelled' ? '' : '販売数量に対応する在庫が戻ります。'}`)) return; const result = props.onDeleteBooth(item.id); if (result) window.alert(result); }}>削除</button></div></article>)}</section>}</>}
    {tab === 'anniversaries' && <AnniversaryManagementPanel campaigns={props.anniversaryCampaigns} shipments={props.anniversaryShipments} onSaveCampaign={props.onSaveAnniversaryCampaign} onSaveShipment={props.onSaveAnniversaryShipment} onSaveQrReference={props.onSaveAnniversaryShipmentQrReference} onDeleteShipment={props.onDeleteAnniversaryShipment} onDeleteCampaign={props.onDeleteAnniversaryCampaign} qrConnection={props.anniversaryQrConnection} onEditingStateChange={onEditingStateChange}/>}
    {tab === 'history' && <section className="inventory-section"><h2>在庫履歴</h2>{[...props.movements].sort((a, b) => b.date.localeCompare(a.date) || b.createdAt.localeCompare(a.createdAt)).map(item => <article className="inventory-row" key={item.id}><strong>{props.products.find(product => product.id === item.productId)?.name ?? '不明な商品'}</strong><span>{item.date}・{item.type === 'boothWarehouseSale' ? 'BOOTH倉庫販売' : item.type === 'boothSale' ? 'BOOTH家発送販売' : item.type}</span><span>{['restock', 'return', 'adjustmentIncrease', 'boothCancellation'].includes(item.type) ? '+' : '-'}{item.quantity}</span><small>{item.memo}</small></article>)}</section>}
    ...
    <dialog ref={dialogRef} className="inventory-dialog" onCancel={(event) => { event.preventDefault(); close(); }}><form key={`${mode}:${editingProduct?.id ?? ''}:${editingEvent?.id ?? ''}:${editingBooth?.id ?? ''}`} onSubmit={submit} noValidate><header><div><p className="eyebrow">Inventory</p><h2>{mode === 'product' ? (editingProduct ? '商品を編集' : '商品を登録') : mode === 'movement' ? '在庫を調整' : mode === 'event' ? (editingEvent ? 'イベント販売を編集' : 'イベント販売を記録') : (editingBooth ? 'BOOTH販売を編集' : 'BOOTH販売を記録')}</h2></div><button type="button" aria-label="ダイアログを閉じる" onClick={close}>×</button></header>{error && <p className="form-error" role="alert">{error}</p>}{mode === 'event' && <fieldset className="inventory-fieldset inventory-status-choice"><legend>記録状態</legend><label className="inventory-check"><input type="radio" checked={eventStatus === 'planned'} onChange={() => setEventStatus('planned')}/>{editingEvent?.status === 'completed' ? '売上入力を取り消す' : '売上未入力として保存'}</label><label className="inventory-check"><input type="radio" checked={eventStatus === 'completed'} onChange={() => setEventStatus('completed')}/>売上入力済みとして保存</label>{eventStatus === 'planned' && <p>売上未入力では販売数・サンプル数を保存せず、在庫を減らしません。</p>}{editingEvent?.status === 'completed' && eventStatus === 'planned' && <p className="inventory-warning">この商品の売上入力を取り消すと、販売数とサンプル数に対応する在庫が戻り、状態は「売上未入力」になります。</p>}</fieldset>}
      ...
      <footer><button type="button" onClick={close}>キャンセル</button><button className="health-primary-button" type="submit" disabled={(mode === 'movement' && isMovementSubmitting) || (mode === 'booth' && isBoothSubmitting)}>{mode === 'movement' && isMovementSubmitting || mode === 'booth' && isBoothSubmitting ? '保存中…' : mode === 'event' ? eventStatus === 'planned' ? editingEvent?.status === 'completed' ? '売上入力を取り消す' : '売上未入力として保存' : editingEvent ? '売上を修正' : '売上を保存' : '保存'}</button></footer></form></dialog>
  </div>;
}
