...
import { calculateAllProductStocks, calculateCurrentStock, canDecreaseStock } from '../utils/inventoryCalculation';
import { toDateKey } from '../utils/date';
import { EventQuickCreateFields, type QuickEventDraft } from './EventQuickCreateFields';
type Tab = 'stock' | 'products' | 'events' | 'booth' | 'history';
...
interface Props {
    ...
    onSaveBooth: (value: BoothSalesRecord) => string | null;
...
export function InventoryPage(props: Props) {
    ...
    const close = () => { dialogRef.current?.close(); setEditingEvent(null); setEditingBooth(null); setError(''); setEventFieldErrors({}); };
    ...
    const submit = (submitEvent: React.FormEvent<HTMLFormElement>) => {
        ...
        if (mode === 'movement') {
            ...
            if (type === 'adjustmentDecrease' && !canDecreaseStock(product, props.movements, quantity)) {
                setError(`在庫が不足しています（現在庫 ${calculateCurrentStock(product, props.movements)}個）。`);
                return;
            }
            if (!memo) {
        ...
        if (mode === 'event') {
            ...
            const result = props.onSaveEvent({ id: editingEvent?.id ?? crypto.randomUUID(), eventId, productId, productNameSnapshot: editingEvent?.productNameSnapshot ?? product.name, unitPriceSnapshot: price, broughtQuantity: brought, soldQuantity: sold, sampleQuantity: sample, status: eventStatus, memo: String(data.get('memo') ?? '').trim(), updatedAt: now }, movementDate);
            if (result) {
        ...
        const result = props.onSaveBooth({ id: editingBooth?.id ?? crypto.randomUUID(), date: String(data.get('date')), productId, productNameSnapshot: editingBooth?.productNameSnapshot ?? (product.boothDisplayName || product.name), unitPriceSnapshot: price, quantity, orderReference: String(data.get('orderReference') ?? '').trim(), status: String(data.get('status')) as BoothSalesRecord['status'], memo: String(data.get('memo') ?? '').trim(), createdAt: editingBooth?.createdAt ?? now, updatedAt: now });
        if (result) {
            setError(result);
            return;
        }
    ...
    <nav className="inventory-tabs" aria-label="販売・在庫画面">{([['products', '在庫・商品'], ['events', 'イベント販売'], ['booth', 'BOOTH販売'], ['history', '在庫履歴']] as [
            Tab,
            string
        ][]).map(([value, label]) => <button key={value} aria-pressed={tab === value} onClick={() => setTab(value)}>{label}</button>)}</nav>
    {(tab === 'stock' || tab === 'products') && <><div className="inventory-toolbar"><label>商品検索<input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="商品名・カテゴリ"/></label><label className="inventory-check"><input type="checkbox" checked={showInactive} onChange={(event) => setShowInactive(event.target.checked)}/>販売終了も表示</label><button className="health-primary-button" onClick={() => open('product')}>商品を登録</button></div><div className="inventory-grid">{filteredProducts.map(product => { const first = props.events.find(event => event.id === product.firstSaleEventId); return <article className="inventory-card" key={product.id}><div><span className={`inventory-status ${product.isActive ? 'active' : 'inactive'}`}>{product.isActive ? '販売中' : '販売終了'}</span><h2>{product.name}</h2><p>{product.category || 'カテゴリなし'}</p></div><strong className="inventory-stock">{stocks.get(product.id) ?? 0}<small>個</small></strong><dl><div><dt>通常価格</dt><dd>{money(product.defaultPrice)}</dd></div><div><dt>BOOTH枠</dt><dd>{product.boothListingQuantity ?? '未設定'}</dd></div></dl>{first ? <p className="inventory-first-sale">初売り：{first.date} {first.title}［{first.category}］</p> : product.firstSaleEventId ? <p className="form-error">初売りイベントの予定が見つかりません</p> : null}<div className="inventory-card-actions"><button onClick={() => open('movement', product)}>在庫を調整</button><button onClick={() => open('product', product)}>編集</button></div></article>; })}{!filteredProducts.length && <p className="inventory-empty">登録済みの商品はありません。</p>}</div></>}
    {tab === 'events' && <section className="inventory-section"><div className="inventory-section-heading"><div><h2>イベント販売</h2><p>準備中は持込予定だけを保存し、実績確定時だけ在庫へ反映します。</p></div><button className="health-primary-button" disabled={!props.products.some(item => item.isActive)} onClick={() => open('event')}>イベント販売を記録</button></div><div className="inventory-event-summary"><span>登録商品 {props.eventSales.length}件</span><span>持込予定 {props.eventSales.reduce((sum, item) => sum + item.broughtQuantity, 0)}個</span><span>実績入力済み {completedEvents.length}件</span><span>確定売上 {money(totalEvent)}</span></div>{props.eventSales.map(item => {
                const completed = item.status === 'completed';
                const sold = item.soldQuantity ?? 0;
                const sample = item.sampleQuantity ?? 0;
                return <article className="inventory-row" key={item.id}><strong>{item.productNameSnapshot}</strong><span className={`inventory-status ${completed ? 'active' : 'planned'}`}>{completed ? '実績入力済み' : '準備中'}</span><span>持込{completed ? '数' : '予定'} {item.broughtQuantity}個</span><span>{completed ? `販売 ${sold}／サンプル ${sample}／残 ${item.broughtQuantity - sold - sample}` : '販売数・サンプル・残数：未確定'}</span><span>{completed ? money(sold * item.unitPriceSnapshot) : '売上：未確定'}</span><button onClick={() => open('event', props.products.find(product => product.id === item.productId) ?? null, item)}>編集</button><button className="danger" onClick={() => {
                        if (window.confirm('このイベント販売記録を削除しますか？'))
                            props.onDeleteEvent(item.id);
                    }}>削除</button></article>;
            })}</section>}
    {tab === 'booth' && <section className="inventory-section"><div className="inventory-section-heading"><div><h2>BOOTH販売</h2><p>手動入力です。購入者名・住所・メールは保存しません。</p></div><button className="health-primary-button" disabled={!props.products.some(item => item.isActive && item.boothEnabled)} onClick={() => open('booth')}>BOOTH販売を記録</button></div><p className="inventory-total">有効売上：{money(totalBooth)}</p>{props.boothSales.map(item => <article className="inventory-row" key={item.id}><strong>{item.productNameSnapshot}</strong><span>{item.status === 'pending' ? '未発送' : item.status === 'shipped' ? '発送済み' : 'キャンセル'}・{item.quantity}個</span><span>{item.status === 'cancelled' ? '売上対象外' : money(item.quantity * item.unitPriceSnapshot)}</span><button onClick={() => open('booth', props.products.find(product => product.id === item.productId) ?? null, null, item)}>編集</button></article>)}</section>}
