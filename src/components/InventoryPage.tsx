...
export function InventoryPage(props: Props) {
    ...
    {tab === 'events' && <section className="inventory-section"><div className="inventory-event-registration inventory-actions" aria-label="イベント商品の登録"><button className="health-primary-button" disabled={!props.products.some(item => item.isActive)} onClick={() => { onEditingStateChange?.(true); setEventBatchMode('create'); setIsEventBatchOpen(true); }}>イベント商品の登録</button><button disabled={!props.products.some(item => item.isActive)} onClick={() => open('event')}>追加で登録</button></div><div className="inventory-event-groups">{eventSalesGroups.map((group) => {
