...
interface Props {
    ...
    onSaveEventBatch: (input: { eventId: string; eventDate: string; status: EventSalesRecord['status']; rows: EventSalesBatchDraftRow[] }) =>
...
export function InventoryPage(props: Props) {
    ...
    {tab === 'events' && <section className="inventory-section"><div className="inventory-section-heading"><div><h2>イベント販売</h2><p>準備中は持込予定だけを保存し、実績確定時だけ在庫へ反映します。</p></div><div className="inventory-actions"><button className="health-primary-button" disabled={!props.products.some(item => item.isActive)} onClick={() => { onEditingStateChange?.(true); setIsEventBatchOpen(true); }}>イベント商品をまとめて登録</button><button disabled={!props.products.some(item => item.isActive)} onClick={() => open('event')}>予定作成・単品入力</button></div></div><div className="inventory-event-summary"><span>登録商品 {props.eventSales.length}件</span><span>持込予定 {props.eventSales.reduce((sum, item) => sum + item.broughtQuantity, 0)}個</span><span>実績確定済み {completedEvents.length}件</span><span>確定売上 {money(totalEvent)}</span></div>{props.eventSales.map(item => {
    ...
    <EventSalesBatchDialog open={isEventBatchOpen} products={props.products} events={sortedEvents} records={props.eventSales} initialEventId={selectedEventId} onClose={() => { setIsEventBatchOpen(false); onEditingStateChange?.(false); }} onSave={props.onSaveEventBatch}/>
