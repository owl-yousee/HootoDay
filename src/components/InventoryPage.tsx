    ...
    {tab === 'events' && <section className="inventory-section"><div className="inventory-section-heading"><div><h2>イベント販売</h2><p>売上未入力では持込予定だけを保存し、売上入力時だけ在庫へ反映します。</p></div><div className="inventory-actions"><button className="health-primary-button" disabled={!props.products.some(item => item.isActive)} onClick={() => { onEditingStateChange?.(true); setEventBatchMode('create'); setIsEventBatchOpen(true); }}>イベント商品をまとめて登録</button><button disabled={!props.products.some(item => item.isActive)} onClick={() => open('event')}>予定作成・単品入力</button></div></div><div className="inventory-event-summary"><span>商品 {eventProductCount}種類</span><span>持込予定 {props.eventSales.reduce((sum, item) => sum + item.broughtQuantity, 0)}個</span><span>販売総数 {totalEventSold}個</span><span>売上 {money(totalEvent)}</span></div><div className="inventory-event-groups">{eventSalesGroups.map((group) => {
                ...
                return <article key={group.eventId} className="inventory-event-group">
                  ...
                  <div className="inventory-event-group-summary">
                    {group.planned.length > 0 && <><span>登録商品 <strong>{records.length}種類</strong></span><span>持込合計 <strong>{records.reduce((sum, item) => sum + item.broughtQuantity, 0)}個</strong></span></>}
                    {isMixed && <><span>入力済み <strong>{group.completed.length}種類</strong></span><span>未入力 <strong>{group.planned.length}種類</strong></span></>}
                    {group.completed.length > 0 && <><span>販売数 <strong>{completedSold}個</strong></span><span>売上 <strong>{money(completedSales)}</strong></span></>}
                  </div>
                  <button className="health-primary-button inventory-event-primary-action" onClick={() => {
                      if (group.planned.length > 0) {
                          ...
                          setExpandedEventId(actionsExpanded ? null : group.eventId);
                      }
                  }}>{group.planned.length > 0 ? (isMixed ? '売上未入力の商品を入力' : '売上をまとめて入力') : '商品ごとの売上を修正'}</button>
                  {group.planned.length > 0 && records.length > 1 && <button className="inventory-event-secondary-action" aria-expanded={actionsExpanded} onClick={() => setExpandedEventId(actionsExpanded ? null : group.eventId)}>{actionsExpanded ? '商品別の操作を閉じる' : '商品別の操作を表示'}</button>}
                  <div className="inventory-event-products" aria-label="商品別の売上">
                    {records.map((item) => {
                        ...
                        return <section className="inventory-event-product" key={item.id}>
                          ...
                          {(actionsExpanded || records.length === 1) && <div className="inventory-event-product-actions"><button onClick={() => open('event', props.products.find(product => product.id === item.productId) ?? null, item)}>{completed ? '売上を修正' : '売上を入力'}</button><button className="danger" onClick={() => {
