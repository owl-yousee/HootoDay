...
export function InventoryPage(props: Props) {
    ...
    {tab === 'events' && <section className="inventory-section"><div className="inventory-section-heading"><div><h2>イベント販売</h2><p>準備中は持込予定だけを保存し、実績確定時だけ在庫へ反映します。</p></div><div className="inventory-actions"><button className="health-primary-button" disabled={!props.products.some(item => item.isActive)} onClick={() => { onEditingStateChange?.(true); setEventBatchMode('create'); setIsEventBatchOpen(true); }}>イベント商品をまとめて登録</button><button disabled={!props.products.some(item => item.isActive)} onClick={() => open('event')}>予定作成・単品入力</button></div></div><div className="inventory-event-summary"><span>登録商品 {props.eventSales.length}件</span><span>持込予定 {props.eventSales.reduce((sum, item) => sum + item.broughtQuantity, 0)}個</span><span>実績確定済み {completedEvents.length}件</span><span>確定売上 {money(totalEvent)}</span></div><div className="inventory-event-groups">{eventSalesGroups.map((group) => <article key={group.eventId} className="inventory-event-group"><div><strong>{group.event?.title ?? '予定が見つからないイベント'}</strong><span>{group.event?.date ?? ''}</span><small>準備中 {group.planned.length}件・実績確定済み {group.completed.length}件</small></div>{group.planned.length > 0 && <button className="health-primary-button" onClick={() => { setSelectedEventId(group.eventId); setEventBatchMode('complete'); onEditingStateChange?.(true); setIsEventBatchOpen(true); }}>販売実績をまとめて入力</button>}</article>)}</div>{props.eventSales.map(item => {
