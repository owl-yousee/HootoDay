...
export function InventoryPage(props: Props) {
    ...
    const salesSummaries = useMemo(() => new Map(props.products.map((product) => [
        product.id,
        calculateProductSalesSummary(product.id, props.eventSales, props.boothSales),
    ])), [props.products, props.eventSales, props.boothSales]);
    ...
    {tab === 'products' && <><div className="inventory-toolbar"><label>商品検索<input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="商品名・カテゴリ"/></label><label className="inventory-check"><input type="checkbox" checked={showInactive} onChange={(event) => setShowInactive(event.target.checked)}/>販売終了も表示</label><button className="health-primary-button" onClick={() => open('product')}>商品を登録</button></div><div className="inventory-grid">{filteredProducts.map(product => { const first = props.events.find(event => event.id === product.firstSaleEventId); const summary = salesSummaries.get(product.id) ?? { eventSoldQuantity: 0, eventSalesAmount: 0, boothSoldQuantity: 0, boothSalesAmount: 0 }; return <article className="inventory-card" key={product.id}><div><span className={`inventory-status ${product.isActive ? 'active' : 'inactive'}`}>{product.isActive ? '販売中' : '販売終了'}</span><h2>{product.name}</h2><p>{product.category || 'カテゴリなし'}</p></div><strong className="inventory-stock">{stocks.get(product.id) ?? 0}<small>個</small></strong><dl><div><dt>通常価格</dt><dd>{money(product.defaultPrice)}</dd></div><div><dt>BOOTH枠</dt><dd>{product.boothListingQuantity ?? '未設定'}</dd></div></dl><div className="inventory-sales-summary" aria-label="販売累計"><div><span>イベント販売</span><strong>{summary.eventSoldQuantity.toLocaleString('ja-JP')}個</strong><small>{money(summary.eventSalesAmount)}</small></div><div><span>BOOTH販売</span><strong>{summary.boothSoldQuantity.toLocaleString('ja-JP')}個</strong><small>{money(summary.boothSalesAmount)}</small></div></div>{first ? <p className="inventory-first-sale">初売り：{first.date} {first.title}［{first.category}］</p> : product.firstSaleEventId ? <p className="form-error">初売りイベントの予定が見つかりません</p> : null}<div className="inventory-card-actions"><button onClick={() => open('movement', product)}>在庫を調整</button><button onClick={() => open('product', product)}>編集</button></div></article>; })}{!filteredProducts.length && <p className="inventory-empty">登録済みの商品はありません。</p>}</div></>}
