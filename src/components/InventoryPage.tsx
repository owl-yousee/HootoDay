...
export function InventoryPage(props: Props) {
    ...
    {tab === 'events' && <section className="inventory-section"><div className="inventory-section-heading"><div><h2>イベント販売</h2><p>準備中は持込予定だけを保存し、実績確定時だけ在庫へ反映します。</p></div><button className="health-primary-button" disabled={!props.products.some(item => item.isActive)} onClick={() => open('event')}>イベント販売を記録</button></div><div className="inventory-event-summary"><span>登録商品 {props.eventSales.length}件</span><span>持込予定 {props.eventSales.reduce((sum, item) => sum + item.broughtQuantity, 0)}個</span><span>実績確定済み {completedEvents.length}件</span><span>確定売上 {money(totalEvent)}</span></div>{props.eventSales.map(item => {
                const completed = item.status === 'completed';
                const sold = item.soldQuantity ?? 0;
                const sample = item.sampleQuantity ?? 0;
                return <article className="inventory-row" key={item.id}><strong>{item.productNameSnapshot}</strong><span className={`inventory-status ${completed ? 'active' : 'planned'}`}>{completed ? '実績確定済み' : '準備中'}</span><span>持込{completed ? '数' : '予定'} {item.broughtQuantity}個</span><span>{completed ? `販売 ${sold}／サンプル ${sample}／残 ${item.broughtQuantity - sold - sample}` : '販売数・サンプル・残数：未確定'}</span><span>{completed ? money(sold * item.unitPriceSnapshot) : '売上：未確定'}</span><button onClick={() => open('event', props.products.find(product => product.id === item.productId) ?? null, item)}>{completed ? '販売実績を編集' : '販売実績を入力'}</button><button className="danger" onClick={() => {
