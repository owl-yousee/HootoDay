...
import { buildDayMemoPullPreview, type DayMemoPullPreviewBuildResult } from '../utils/dayMemoPullPreview'
...
interface PullPreviewData extends DayMemoPullPreviewBuildResult {
  ...
  const pullPreview = useCallback(async (options?: { requireConfirmedMetadata?: boolean }) => {
    ...
    for (let page = 0; page < MAX_PAGES; page += 1) {
      ...
      if (data.length < PAGE_LIMIT) {
        const nextPreview = buildDayMemoPullPreview(records, dayMemos, reactMetadata)
        if (!nextPreview) {
          setPreviewState('validation_error')
          setSafeErrorMessage(messageForState('validation_error'))
          return
