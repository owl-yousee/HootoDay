...
export interface DayMemoConflictPreviewItem {
  ...
  pendingStatus: DayMemoPendingOperationV5['status'] | 'intent_recorded'
  checkedAt: string
}
...
export interface DayMemoConflictAdoptionSnapshot {
  ...
  localDeleteIntents: DayMemoSyncMetadataV5['localDeleteIntents']
...
function deleteIntentEvidence(metadata: DayMemoSyncMetadataV5, memos: DayMemo[], date: string): LocalEvidence {
...
function inspectLocalEvidence(metadata: DayMemoSyncMetadataV5, memos: DayMemo[]): LocalEvidence[] {
...
export function useDayMemoConflictPreview({ dayMemos, isConfigured, isSignedIn, connection }: UseDayMemoConflictPreviewInput) {
  ...
  const reset = useCallback(() => {
    ...
    if (loaded.status !== 'ready' || loaded.metadata.version !== 5 || loaded.metadata.workspaceId !== connection.workspaceId) {
      setState('unavailable')
      return
    }
  ...
  const checkConflicts = useCallback(async () => {
    ...
    if (after.status !== 'ready' || after.metadata.version !== 5 || after.raw !== before.raw
