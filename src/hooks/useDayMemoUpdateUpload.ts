...
function metadataMatchesPreview(metadata: DayMemoSyncMetadataV5, raw: string, preview: DayMemoUpdateUploadCandidateSnapshot): boolean {
...
export function useDayMemoUpdateUpload({
  ...
  const uploadPrepared = useCallback(async () => {
    ...
    const loaded = loadDayMemoSyncMetadataAny(window.localStorage)
    const expectedPending = loaded.status === 'ready' && loaded.metadata.version === 5 && loaded.metadata.pendingOperation?.kind === 'upsert' ? loaded.metadata.pendingOperation : null
