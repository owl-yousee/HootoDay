...
function metadataMatchesPreview(metadata: DayMemoSyncMetadataV2, raw: string, preview: DayMemoLocalOnlyUploadCandidateSnapshot): boolean {
...
function remoteMatchesBaselines(metadata: DayMemoSyncMetadataV2, records: Awaited<ReturnType<typeof pullAllDayMemoSyncRecords>> extends { records: infer R } ? Exclude<R, null> : never): boolean {
...
export function useDayMemoLocalOnlyUpload({
  ...
  const uploadPrepared = useCallback(async () => {
    ...
    const loaded = loadDayMemoSyncMetadataAny(window.localStorage)
    const pending = loaded.status === 'ready' && loaded.metadata.version === 2 ? loaded.metadata.pendingOperation : null
    ...
    const sending: DayMemoSyncMetadataV2 = { ...loaded.metadata, pendingOperation: { ...pending, status: 'sending' } }
    ...
    try {
      ...
      const unknown: DayMemoSyncMetadataV2 = { ...sending, pendingOperation: { ...sending.pendingOperation!, status: 'response_unknown' } }
    ...
    if (isConflictDayMemoSyncResult(normalized, connection.workspaceId, memo.date)) {
      const conflict: DayMemoSyncMetadataV2 = { ...sending, pendingOperation: { ...sending.pendingOperation!, status: 'conflict' } }
