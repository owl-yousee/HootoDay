...
export function useDayMemoRemoteTombstoneAdoption({
  ...
  const applyRemoteTombstone = useCallback(async () => {
    ...
    if (!snapshot
      ...
      || loaded.status !== 'ready' || loaded.metadata.version !== 3
    ...
    if (!liveEligibility.current.isConfigured || !liveEligibility.current.isSignedIn
      ...
      || afterPullMetadata.status !== 'ready' || afterPullMetadata.metadata.version !== 3
    ...
    const completedMetadata: DayMemoSyncMetadataV3 = {
      ...loaded.metadata,
      baselines: {
    ...
    if (!isDayMemoSyncMetadataV3(completedMetadata)) {
